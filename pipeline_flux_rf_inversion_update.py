...
class RFInversionFluxPipeline(
    ...
    def encode_image(self, image, mask_image=None, masked_image_latents=None, dtype=None, height=None, width=None, resize_mode="default", crops_coords=None):
        ...
        if mask_image is not None:
            mask_condition = self.image_processor.preprocess(
                image=mask_image, height=height, width=width, resize_mode=resize_mode, crops_coords=crops_coords
            )
            if mask_condition.shape[1] == 3:  # If mask has 3 channels, convert to grayscale
                mask_condition = mask_condition.mean(dim=1, keepdim=True)
            mask_condition = torch.where(
                mask_condition > 0.5,
                torch.ones_like(mask_condition),  # Use ones_like for proper tensor dtype/device
                torch.zeros_like(mask_condition) # Use zeros_like for proper tensor dtype/device
            )
        ...
        if masked_image_latents is None:
          ...
          # masked_image = image * (1 - mask_condition)
        else:
        ...
        x0 = x0.to(dtype)
        return x0, resized, masked_image, mask_condition
  ...
  #      if height % self.vae_scale_factor != 0 or width % self.vae_scale_factor != 0:
      ...
      #      )
        ...
        if mask_latents is not None:
            if image_latents is None:
                raise ValueError(
                    "If `mask_latents` are provided, `image_latents` must also be passed. Masking requires image latents."
                )
            if not isinstance(mask_latents, torch.Tensor):
                raise TypeError(f"`mask_latents` must be a tensor, but got {type(mask_latents)}")
            if mask_latents.shape != image_latents.shape:
                raise ValueError(
                    f"`mask_latents` must have the same shape as `image_latents`, but got {mask_latents.shape} vs {image_latents.shape}"
                )
    ...
    def prepare_mask_latents(
        ...
        # and half precision
        mask = torch.nn.functional.interpolate(mask, size=(height, width), mode="bilinear", align_corners=False)  # Use bilinear interpolation for smooth transitions
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                if do_rf_inversion:
                    ...
                    #eta_map = eta_black + (eta_white - eta_black) * mask_latents  # Different eta per pixel

                    #eta_white_t = eta_white * (1 - i / num_inference_steps) ** eta_decay_power if decay_eta else eta_white
                    
                   # eta_map = torch.where(
                    #    mask_latents > 0.5, 
                    #    torch.tensor(eta_white, dtype=latents.dtype, device=latents.device), 
                   #     torch.tensor(eta_black, dtype=latents.dtype, device=latents.device)
                   # )

                    eta_white_t = torch.tensor(eta_white, dtype=mask_latents.dtype, device=mask_latents.device)
                    eta_black_t = torch.tensor(eta_black, dtype=mask_latents.dtype, device=mask_latents.device)

                    eta_map = eta_white_t + (eta_black_t - eta_white_t) * (1 - mask_latents)

                   # eta_t = eta_map if start_timestep <= i < stop_timestep else 0.0
                    ...
                    if start_timestep <= i < stop_timestep:
                        ...
                        eta_t = torch.where(
                            mask_latents == 1.0, 
                            torch.tensor(0.4*(1 - i / num_inference_steps) ** 2, dtype=latents.dtype, device=latents.device),  # White
                            torch.where(
                                mask_latents == 0.0, 
                                torch.tensor(0.9, dtype=latents.dtype, device=latents.device),  # Black
                                torch.tensor(0.9, dtype=latents.dtype, device=latents.device)   # Grayscale
                            )
                        )
                        
                      #  eta_t = torch.zeros_like(eta_map)  # Zero out eta if outside range
                   ...
                   # eta_t = eta_black if start_timestep <= i < stop_timestep else 0.0
                    #if decay_eta:
                     #   eta_t = eta_t * (1 - i / num_inference_steps) ** eta_decay_power  # Decay eta over the loop
                    #v_hat_t = v_t + eta_t * (v_t_cond - v_t)
                    ...
                    if mask_latents is not None:
                        #latents = latents * (1 - mask_latents) + (latents + v_hat_t * (sigmas[i] - sigmas[i + 1])) * mask_latents
    ...
    def invert(
        ...
        strength: float = 1.0,
        gamma_white: float = 0.5,
        gamma_black: float = 0.5,
        height: Optional[int] = None,
        width: Optional[int] = None,
        timesteps: List[int] = None,
        dtype: Optional[torch.dtype] = None,
        ...
        # 1. prepare image
        image_latents, _, masked_image, mask_condition = self.encode_image(image, mask_image, masked_image_latents, height=height, width=width, dtype=dtype)
       

        mask_latents = None
        if mask_image is not None:
           # 1.1. Obtain latent representations of the mask and masked image
            mask_latents, masked_image_latents = self.prepare_mask_latents(
                mask_condition, masked_image, batch_size, num_channels_latents, height, width, dtype, device
            )
        ...
        with self.progress_bar(total=N - 1) as progress_bar:
            for i in range(N - 1):
                ...
                # Eq 8 dY_t = [u_t(Y_t) + γ(u_t(Y_t|y_1) - u_t(Y_t))]dt
                #gamma_map = gamma_black + (gamma_white - gamma_black) * mask_latents
               # gamma_map = torch.where(
               #     mask_latents > 0.5, 
              #      torch.tensor(gamma_white, dtype=Y_t.dtype, device=Y_t.device), 
               #     torch.tensor(gamma_black, dtype=Y_t.dtype, device=Y_t.device)
              #  )

                gamma_white_t = torch.tensor(gamma_white, dtype=mask_latents.dtype, device=mask_latents.device)
                gamma_black_t = torch.tensor(gamma_black, dtype=mask_latents.dtype, device=mask_latents.device)

                gamma_map = gamma_white_t + (gamma_black_t - gamma_white_t) * (1 - mask_latents)

                
                u_hat_t_i = u_t_i + gamma_map * (u_t_i_cond - u_t_i)

                if mask_latents is not None:
                    # Apply inversion only on masked areas
                    #Y_t = Y_t * (1 - mask_latents) + (Y_t + u_hat_t_i * (sigmas[i] - sigmas[i + 1])) * mask_latents
        ...
        return Y_t, image_latents, latent_image_ids, mask_latents
